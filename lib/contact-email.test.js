...
describe('SendGrid contact email', () => {
  ...
  it('uses the hard-coded Anthem recipients', () => {
    vi.stubEnv('SENDGRID_FROM_EMAIL', '');

    expect(getSendGridConfig()).toMatchObject({
      fromEmail: 'contact@anthemcv.com',
      toEmail: 'anthemcustomers@anthemcv.com',
      ccEmail: 'aaronson@anthemcv.com',
    });
  });
  ...
  it('sends text and escaped HTML content to SendGrid', async () => {
    ...
    expect(body.personalizations[0].to[0].email).toBe('anthemcustomers@anthemcv.com');
    expect(body.personalizations[0].cc[0].email).toBe('aaronson@anthemcv.com');
    ...
    expect(body.content[1].value).toContain('align="center" bgcolor="#f0162d" style="padding:28px 24px 24px;background-color:#f0162d');
    expect(body.content[1].value).toContain('border:1px solid #07101f');
    expect(body.content[1].value).toContain('width="310"');
    expect(body.content[1].value).toContain('align="center" bgcolor="#f0162d" style="background-color:#f0162d;"');
